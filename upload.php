 ...
 */
function createFileFromChunks($temp_dir, $fileName, $chunkSize, $totalSize, $saveTo) {
    ...
    if ($total_files * $chunkSize >= ($totalSize - $chunkSize + 1)) {
        ...
        if (($fp = fopen($saveTo . $fileName, 'w')) !== false) {
...
if (!empty($_FILES)) foreach ($_FILES as $file) {
    ...
    if (!move_uploaded_file($file['tmp_name'], $dest_file)) {
        ...
        createFileFromChunks($temp_dir, $_POST['flowFilename'],
            $_POST['flowChunkSize'], $_POST['flowTotalSize'], 'temp/');
        echo 'temp/' . $_POST['flowFilename'];
    }
}